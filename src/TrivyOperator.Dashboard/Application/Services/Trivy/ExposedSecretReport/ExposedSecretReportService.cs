    ...
    : IExposedSecretReportService
{
    ...
    public Task<IEnumerable<EsSeveritiesByNsSummaryDto>> GetExposedSecretReportSummaryDtos()
    {
        ...
        IEnumerable<ExposedSecretReportCr> cachedValues = [.. cache.SelectMany(kvp => kvp.Value.Values)];
        ...
        IEnumerable<EsSeveritiesByNsSummaryDetailDto> totalSummary = cachedValues.SelectMany(
        ...
        var detailDtos = totalSummary
            .Concat(severityIds
                .Where(id => !totalSummary.Any(x => x.Id == id))
